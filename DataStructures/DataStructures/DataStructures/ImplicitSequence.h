...
namespace ds::amt {
	...
	auto ImplicitSequence<DataType>::access(size_t index) const -> BlockType*
	{
		...
		// po implementacii vymazte vyhodenie vynimky!
		if (index < size()) {
			return &(this->getMemoryManager()->getBlockAt(index)); //chcem ukazovatel na to pamatove miesto
		}
		else {
			return nullptr;
		}
	...
	auto ImplicitSequence<DataType>::insertFirst() -> BlockType&
	{
		// TODO 03
		// po implementacii vymazte vyhodenie vynimky!
		this->*(getMemoryManager()->allocateMemoryAt(0));
		//throw std::runtime_error("Not implemented yet");
	}
	...
	void ImplicitSequence<DataType>::removeLast()
	{
		// TODO 03
		// po implementacii vymazte vyhodenie vynimky!
		this->getMemoryManager()->releaseMemoryAt(-1);
		//throw std::runtime_error("Not implemented yet");
	}
