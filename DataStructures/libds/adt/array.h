...
namespace ds::adt {
	...
	{
		const Array<T>* otherSequence = dynamic_cast<const Array<T>*>(&other);

        if (!otherSequence) throw std::invalid_argument("Wrong type of ADT!");
        if (base_ != otherSequence->base_ || size() != otherSequence->size()) throw std::invalid_argument("Dimensions do not match!");

        ADS<T>::assign(*otherSequence);
	...
	{
		if (!this->validateIndex(index)) throw std::out_of_range("Wrong index!");
